...
@pytest.mark.parametrize(
    "creds_file_name,creds_file_content,patches,expected_feature_id",
    [
        (
            'shared_credentials',
            '[default]\naws_access_key_id = FAKEACCESSKEY\naws_secret_access_key = FAKESECRET',
...
def test_user_agent_has_file_based_feature_ids(
    ...
    credentials_file = tmp_path / creds_file_name
    credentials_file.write_text(creds_file_content)

    creds_env_var = None
    if creds_file_name == 'shared_credentials':
        creds_env_var = 'AWS_SHARED_CREDENTIALS_FILE'
    elif creds_file_name == 'config':
        creds_env_var = 'AWS_CONFIG_FILE'
